...
const TIPO_COLORS: Record<string, string> = {
  ...
  Registro: "bg-slate-100 text-slate-600",
};
...
function groupByDate(items: AndamentoDisplay[]) {
  const groups: Record<string, AndamentoDisplay[]> = {};
      ...
      <div>
        ...
        <p className="mt-1 font-body text-sm text-muted">
          {counts.nao_lidos > 0
            ...
            : andamentos.length > 0
              ? `${andamentos.length} andamentos registrados`
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
        ...
        {andamentos.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-14 text-center">
            ...
            <p className="max-w-xs font-body text-xs text-slate-400">
              Os andamentos aparecerão aqui quando forem adicionados nas páginas
              dos processos.
            </p>
            <Link
          ...
          <div className="divide-y divide-border">
            {Object.entries(groups).map(([data, andamentosList]) => (
              <div key={data}>
