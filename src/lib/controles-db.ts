...
function mapRow(r: any): Controle {
  return {
    ...
    fatal: !!r.fatal,
    pontos: r.pontos != null ? Number(r.pontos) : 1,
...
export async function getControles(
  ...
  const rows = await sql`
    SELECT
      ...
      c.prioridade, c.fatal, c.pontos, c.publicacao_id, c.tipo_demanda, c.observacoes, c.prazo_interno::text, c.dados, c.checklist, c.created_at::text
    FROM controles c
