...
async function getAgendaProxima(dias: number): Promise<AgendaItem[]> {
  const rows = await sql`
    SELECT c.tipo, c.descricao, c.data_evento::text, c.prioridade,
  ...
  `;
  return rows.map((r) => ({
    tipo: TIPO_LABEL[r.tipo] ?? String(r.tipo),
 ...
 * de permissão: agenda/audiências exige "controles:ver", equipe exige
 ...
 * Sem tool-calling: busca tudo de uma vez antes da chamada, suficiente pro
 * volume de dados de um escritório.
 */
export async function buildIrisContextText(
  ...
  const agendaTxt =
    agenda === null
      ? "(sem acesso a agenda/audiências/prazos com o perfil deste usuário)"
      : agenda.length === 0
        ? "Nada agendado nos próximos 30 dias."
        : agenda
            .map(
              (a) =>
                `${a.data} — ${a.tipo}: ${a.descricao}${a.clienteNome ? ` (cliente: ${a.clienteNome})` : ""}${a.prioridade ? ` [prioridade ${a.prioridade}]` : ""}`
            )
            .join("\n");
  ...
  return `━━━ AGENDA — próximos 30 dias (audiências, prazos, perícias, DCB, benefícios, alvarás) ━━━
${agendaTxt}
